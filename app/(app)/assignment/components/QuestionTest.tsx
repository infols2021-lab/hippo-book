...
export default function QuestionTest({ question, value, onChange, disabled }: Props) {
  ...
  if (options.length === 0) {
    return <div style={{ color: "rgba(0,0,0,0.5)", fontStyle: "italic" }}>Нет вариантов ответа</div>;
  }
        ...
        return (
          <label
            ...
            style={{
              ...
              border: `2px solid ${isSelected ? "#007bff" : "rgba(0,0,0,0.08)"}`,
              cursor: disabled ? "default" : "pointer",
              transition: "all 0.2s ease",
              boxShadow: isSelected ? "0 4px 12px rgba(0, 123, 255, 0.1)" : "0 2px 4px rgba(0,0,0,0.02)",
          ...
          >
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "12px" }}>
              {opt.text && (
                <div style={{ fontSize: "16px", color: "#333", lineHeight: 1.4, fontWeight: isSelected ? 600 : 400 }}>
                  {opt.text}
                </div>
              )}

              {/* Рендер медиа для этого варианта ответа */}
